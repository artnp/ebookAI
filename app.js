...
// Screenshot Folder Auto-Rotation Config (GitHub Web UI จำกัดแสดงผลไม่เกิน 1,000 ไฟล์ต่อโฟลเดอร์)
let GITHUB_SCREENSHOT_FOLDER_BASE = 'pdf_screenshot';
let GITHUB_SCREENSHOT_MAX_ITEMS = 950;
...
// ตรวจสอบและวิเคราะห์ Layout ของหน้าทันทีถ้ายังไม่มีแคช เพื่อให้หาภาพประกอบเจอตั้งแต่ครั้งแรกที่เลือก <li>
async function ensurePageLayoutBlocks(pageNum) {
    if (pageLayoutBlocks[pageNum] && pageLayoutBlocks[pageNum].length > 0) {
        return pageLayoutBlocks[pageNum];
    }
    ...
    if (canvas && canvas.width > 0 && canvas.height > 0) {
        try {
            ...
            console.warn(`[ensurePageLayoutBlocks] analyze failed for page ${pageNum}:`, e);
        }
    } else if (pdfDoc) {
        try {
            const page = await pdfDoc.getPage(pageNum);
            ...
            console.warn(`[ensurePageLayoutBlocks] offscreen analyze failed for page ${pageNum}:`, e);
        }
    }
    return pageLayoutBlocks[pageNum] || [];
}
...
async function findBestIllustrationWithRetries(pageNum, textRegion, canvasW, canvasH) {
    ...
    if (!blocks || blocks.length === 0) {
        ...
        blocks = getPageLayoutBlocksAtScale(pageNum, scale);
    }
    const canvas = getRenderedCanvas(pageNum);
        ...
        if (scale < 2.95) {
            ...
            if (!bestImageBlock) {
                ...
                if (bestImageBlock) {
                    ...
                    console.log('[Smart Crop] Image-first (post-render): found image block', Math.round(bestImageBlock.w) + 'x' + Math.round(bestImageBlock.h));
                }
            }
        }
...
async function findLatestGitHubFile(options = null) {
    ...
    while (true) {
        ...
        if (!data) {
            // หากไฟล์ candidate ยังไม่มีอยู่บน GitHub แสดงว่าคือไฟล์ใหม่ที่จะถูกสร้างเพื่อบันทึกข้อมูล
        ...
        // หากไฟล์เต็มแล้ว (>= GITHUB_MAX_ITEMS) หรือไฟล์มีขนาด > 0 แต่อ่าน items ไม่ได้ ให้หมุนขึ้นไฟล์ถัดไป
